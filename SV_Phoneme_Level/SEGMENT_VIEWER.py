...
def add_button(TIMESTEPS, COLOR, CAPTION, NAME):
	...
	linea_out=linea_out+TIMESTEPS+"\" style=\"width:35px;background-color:"
...
#ENDDEF
########################################################################
def compare_timesteps_2(tupla_rufa,tupla_mfa):
	COLOR="green"
	phone_rufa=tupla_rufa[0]
	phone_mfa=tupla_mfa[0]
	...
	if dif_ini > THRES_PHONE_LEVEL:
		COLOR="white"
	#ENDIF	
	dif_fin=round(abs(fin_rufa-fin_mfa),3)
	if dif_fin > THRES_PHONE_LEVEL:
		COLOR="white"
	#ENDIF
	if phone_rufa==phone_mfa and COLOR=="white":
		COLOR="yellow"
	elif phone_rufa!=phone_mfa and COLOR=="green":
		COLOR="gray"
	#ENDIF
	return COLOR
#ENDDEF
########################################################################
def validate_index(lista, index):
...
#ENDEF
########################################################################
def determine_colors_2(lista_rufa_phones,lista_mfa_phones):
	lista_invalid=["n.","SIL","sil","spn"]
	#Determine valid tuples for RUFA
	lista_valid_rufa=[]
	for tupla in lista_rufa_phones:
		phone = tupla[0]
		if phone not in lista_invalid:
	...
	if len_rufa==len_mfa:
		...
		if len_rufa>=len_mfa:
			lista_mas_larga=lista_valid_rufa
			lista_mas_corta=lista_valid_mfa
			mas_larga="RUFA"
		else:
			lista_mas_larga=lista_valid_mfa
			lista_mas_corta=lista_valid_rufa
			mas_larga="MFA"
		#ENDIF
		for index in range(0,len(lista_mas_larga)):
			if validate_index(lista_mas_corta, index)==True:
				tupla_rufa=lista_valid_rufa[index]
				tupla_mfa=lista_valid_mfa[index]
...
archivo_out=open('templates/PHONEME_VIEW.html','w')
...
archivo_out.write("SEGMENT VIEWER: Phoneme View")
archivo_out.write(" </title>\n")
archivo_out.write("</head>\n")

archivo_out.write("<body>\n")

archivo_out.write("<H1>SEGMENT VIEWER: Phoneme View</H1>\n")
...
for clave, nada in LISTA_KEYS:
	...
	lista_rufa_phones=read_tg_items(HASH_RUFA_TG_PATHS[clave],'PHONEME')
	lista_mfa_phones =read_tg_items(HASH_MFA_TG_PATHS[clave],'PHONEME')

	[lista_rufa_colors, lista_mfa_colors]=determine_colors_2(lista_rufa_phones,lista_mfa_phones)
...
archivo_out.write("\t#ENDIF\n")
archivo_out.write("\treturn render_template(\"PHONEME_VIEW.html\")\n")
